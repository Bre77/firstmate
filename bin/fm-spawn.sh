...
#   FM_CREW_MEMORY_SWAP env vars (see --help), or skip the wrap silently with
#   FM_CREW_MEMORY_CAP=off. Falls back to an unwrapped launch, with a stderr
#   warning, when this host has no reachable `systemd-run --user` (no user
#   systemd instance) - the cap never blocks a spawn. Applies to KIND=ship and KIND=scout only; a --secondmate AGENT
...
if [ "$KIND" = secondmate ]; then
  ...
  if [ "${FM_CREW_MEMORY_CAP:-on}" = off ]; then
    :
  elif crew_memory_cap_available; then
