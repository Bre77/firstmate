...
launch_template() {
  ...
  case "$harness" in
    ...
    # --settings '{"attribution":{"commit":""}}' empties claude's own injected
    # "Co-Authored-By: Claude ... <noreply@anthropic.com>" commit trailer
    # (verified: settings schema attribution.commit, empirically confirmed
    # against a real commit with and without the flag). Scoped to this one
    # launch via the CLI flag, same as the prompt-suggestion env var above;
    # never touches the captain's global ~/.claude/settings.json.
    claude) printf '%s' 'CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude --dangerously-skip-permissions --settings '\''{"attribution":{"commit":""}}'\'' __MODELFLAG____EFFORTFLAG__"$(__OPINPUT__ encode launch-brief < __BRIEF__)"' ;;
    codex)
      if [ "$kind" = secondmate ]; then
...
freshen_spawn_worktree_base() {  # <worktree>
  ...
  if ! git -C "$worktree" remote get-url origin >/dev/null 2>&1; then
    echo "BOOTSTRAP_INFO: skipped freshening pooled worktree '$worktree' (no origin remote; local-only project)" >&2
    return 0
  fi
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  #
  # A single read that proves out as a worktree root is still not proof the pane
  # settled there: on some tmux/WSL setups a brand-new window's pane_current_path
  # transiently reports an unrelated stale path (seen live as another real git
  # checkout entirely) before the shell catches up with treehouse get's cd. That
  # stale path passes the PROJ_ABS_REAL comparison, the worktree-root proof, and
  ...
  # worktree= in state/<id>.meta. Require two consecutive reads to agree on the same
  # proven-worktree path before accepting it; a mismatch just becomes the new
  # candidate rather than resetting the wait, so a pane that is already settled by
  # the first real read only costs the one existing inter-poll sleep as
