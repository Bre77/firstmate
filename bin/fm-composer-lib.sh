...
# fm_composer_queued_hint_matches: the ONE unconditional check for the
# fleet-wide "message queued, not yet handed to the agent" hint. Kept
# separate from fm_composer_idle_matches/idle_collision below, which defers
# an ambiguous idle-placeholder match to styling or position proof: the
# queued hint is a fixed, certain string a busy pane shows after genuinely
# accepting a steer, never a placeholder a human could coincidentally type,
# so it must always classify empty and never fall into that deferral.
fm_composer_queued_hint_matches() {  # <content>
  printf '%s' "$1" | grep -qF "$FM_COMPOSER_QUEUED_HINT_RE"
}
...
fm_composer_classify_content() {  # <bordered> <content> [idle_re] [idle_case] [plain_content] [placeholder-position] [styled]
  ...
  fm_composer_queued_hint_matches "$content" && { printf 'empty'; return 0; }
