...
For a KNOWN timed wait (a deploy verification window, a rate-limit reset, an upstream release), name what you are waiting for and until when before going quiet, e.g. \`$PAUSED_VERB: canary deploy verification until 2026-09-12T14:30Z\` - generic \`$PAUSED_VERB:\` text alone is easy to misread as a stuck worker.
...
4. Report status by appending one line:
   ...
   \`$PAUSED_VERB: canary deploy verification until 2026-09-12T14:30Z\` - generic \`$PAUSED_VERB:\` text alone
...
[ "$MODE" = fork-only ] || DOD=$(fm_dod_block "$MODE" "$ID") || exit 1
