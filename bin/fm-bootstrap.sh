...
# shellcheck source=bin/fm-betterstack-lib.sh disable=SC1091
. "$SCRIPT_DIR/fm-betterstack-lib.sh"
# shellcheck source=bin/fm-captain-msg-lib.sh disable=SC1091
...
install_cmd() {
  case "$1" in
    tmux|node|git|gh|curl|jq|orca|python3|zellij) echo "brew install $1  # or the platform's package manager" ;;
...
# ClickStack webhook receiver (opt-in, fork-only): when this home has a
# config/clickstack-webhook.env gate, wire the inbox poll into the EXISTING
# watcher check mechanism without touching fm-watch.sh or any watcher-backbone
# file, exactly like x_mode_setup. Drops one idempotent, gitignored artifact:
#   state/clickstack-watch.check.sh - check shim that execs bin/fm-clickstack-poll.sh
# The listener daemon itself is a separate process armed by firstmate via
# bin/fm-clickstack-arm.sh (this only reports that it should be); bootstrap must
# never block, so it does not start the daemon. On opt-out (no gate) it removes
# the shim so the instance reverts to no-poll behavior. Absent a gate AND with no
# leftover shim it is a complete no-op (nothing written, nothing printed).
clickstack_webhook_setup() {
  ...
  if ! cshook_enabled; then
    if [ -e "$shim" ]; then
      if clickstack_remove_artifacts; then
        echo "CLICKSTACK: webhook receiver off - removed inbox poll shim; stop the listener with bin/fm-clickstack-arm.sh --stop"
      else
        echo "CLICKSTACK: webhook receiver off - failed to remove inbox poll shim"
      fi
    fi
    return 0
  fi
  ...
  if ! command -v python3 >/dev/null 2>&1; then
    ...
    if [ -e "$shim" ] && clickstack_remove_artifacts; then
      echo "CLICKSTACK: webhook receiver off - python3 missing; install it and rerun bootstrap"
    fi
    return 0
  fi

  clickstack_arm_failed() {
    if clickstack_remove_artifacts; then
      echo "CLICKSTACK: webhook receiver off - failed to arm inbox poll shim"
    else
      echo "CLICKSTACK: webhook receiver off - failed to arm inbox poll shim; stale shim remains"
    fi
  }

  mkdir -p "$STATE" 2>/dev/null || { clickstack_arm_failed; return 0; }

  shim_body=$(cat <<EOF
#!/usr/bin/env bash
# Auto-generated by fm-bootstrap.sh - ClickStack webhook inbox poll shim.
...
# BetterStack status-page webhook route (opt-in, fork-only): when this home has
# a config/betterstack-webhook.env gate, wire the inbox poll into the EXISTING
# watcher check mechanism, independently of clickstack_webhook_setup above.
# Drops one idempotent, gitignored artifact:
#   state/betterstack-watch.check.sh - check shim that execs bin/fm-betterstack-poll.sh
# This route shares the ClickStack receiver's listener process and port
# (docs/betterstack-webhook.md), armed via bin/fm-betterstack-arm.sh (this only
# reports that it should be); bootstrap must never block, so it does not start
# the daemon or generate the token itself. On opt-out (no gate) it removes the
# shim. Absent a gate AND with no leftover shim it is a complete no-op.
betterstack_webhook_setup() {
  local shim shim_body
  ...
  if ! bshook_enabled; then
    if [ -e "$shim" ]; then
      if betterstack_remove_artifacts; then
        echo "BETTERSTACK: webhook route off - removed inbox poll shim; stop the shared receiver with bin/fm-clickstack-arm.sh --stop if ClickStack is also off"
      else
        echo "BETTERSTACK: webhook route off - failed to remove inbox poll shim"
      fi
    fi
    return 0
  fi
  ...
  if ! command -v python3 >/dev/null 2>&1; then
    echo "MISSING: python3 (install: $(install_cmd python3))"
    if [ -e "$shim" ] && betterstack_remove_artifacts; then
      echo "BETTERSTACK: webhook route off - python3 missing; install it and rerun bootstrap"
    fi
    return 0
  fi

  betterstack_arm_failed() {
    if betterstack_remove_artifacts; then
      echo "BETTERSTACK: webhook route off - failed to arm inbox poll shim"
    else
      echo "BETTERSTACK: webhook route off - failed to arm inbox poll shim; stale shim remains"
    fi
  }

  mkdir -p "$STATE" 2>/dev/null || { betterstack_arm_failed; return 0; }

  shim_body=$(cat <<EOF
#!/usr/bin/env bash
# Auto-generated by fm-bootstrap.sh - BetterStack webhook inbox poll shim.
# The watcher runs this each check cycle; output becomes a check: wake.
export FM_HOME=$(printf '%q' "$FM_HOME")
...
# Captain messaging inbound-reply route (opt-in, fork-only): when this home has
# a config/captain-msg.env gate, wire the inbox poll into the EXISTING watcher
# check mechanism, independently of the two setups above. Drops one idempotent,
# gitignored artifact:
#   state/captain-msg-watch.check.sh - check shim that execs bin/fm-captain-msg-poll.sh
# This route shares the ClickStack receiver's listener process and port
# (docs/captain-messaging.md), armed via bin/fm-captain-msg-arm.sh (this only
# reports that it should be); bootstrap must never block, so it does not start
# the daemon or fetch the Twilio Account Auth Token itself. Unlike the other two
# integrations, a present gate can still be an incomplete config (missing
# destination/account/messaging-service SID) - that is reported as its own
# diagnostic rather than silently arming a poll shim that would only surface
# failures later. On opt-out (no gate) it removes the shim.
captain_msg_setup() {
  local shim shim_body
  shim="$STATE/captain-msg-watch.check.sh"

  captain_msg_remove_artifacts() {
    rm -f "$shim" "$STATE/captain-msg-watch.check-trust" 2>/dev/null || true
    [ ! -e "$shim" ]
  }

  if ! cmsg_enabled; then
    if [ -e "$shim" ]; then
      if captain_msg_remove_artifacts; then
        echo "CAPTAIN_MSG: messaging channel off - removed inbox poll shim; stop the shared receiver with bin/fm-clickstack-arm.sh --stop if ClickStack and BetterStack are also off"
      else
        echo "CAPTAIN_MSG: messaging channel off - failed to remove inbox poll shim"
      fi
    fi
    return 0
  fi
  ...
  if ! command -v python3 >/dev/null 2>&1; then
    echo "MISSING: python3 (install: $(install_cmd python3))"
    if [ -e "$shim" ] && captain_msg_remove_artifacts; then
      echo "CAPTAIN_MSG: messaging channel off - python3 missing; install it and rerun bootstrap"
    fi
    return 0
  fi

  if ! cmsg_require_config >/dev/null 2>&1; then
    [ -e "$shim" ] && captain_msg_remove_artifacts
    echo "CAPTAIN_MSG: messaging channel off - config/captain-msg.env is incomplete; see docs/captain-messaging.md"
    return 0
  fi

  captain_msg_arm_failed() {
    if captain_msg_remove_artifacts; then
      echo "CAPTAIN_MSG: messaging channel off - failed to arm inbox poll shim"
    else
      echo "CAPTAIN_MSG: messaging channel off - failed to arm inbox poll shim; stale shim remains"
    fi
  }

  mkdir -p "$STATE" 2>/dev/null || { captain_msg_arm_failed; return 0; }

  shim_body=$(cat <<EOF
#!/usr/bin/env bash
# Auto-generated by fm-bootstrap.sh - captain messaging inbox poll shim.
# The watcher runs this each check cycle; output becomes a check: wake.
export FM_HOME=$(printf '%q' "$FM_HOME")
exec $(printf '%q' "$FM_ROOT/bin/fm-captain-msg-poll.sh")
EOF
)
  x_mode_write_if_changed "$shim" "$shim_body" 700 || { captain_msg_arm_failed; return 0; }
  FM_HOME="$FM_HOME" FM_STATE_OVERRIDE="$STATE" "$FM_ROOT/bin/fm-check-register.sh" captain-msg-watch \
    >/dev/null || { captain_msg_arm_failed; return 0; }

  echo "CAPTAIN_MSG: messaging channel on - inbox poll armed via state/captain-msg-watch.check.sh; arm the shared receiver with bin/fm-captain-msg-arm.sh"
}
...
if [ "${FM_BOOTSTRAP_DETECT_ONLY:-0}" != 1 ]; then
  ...
  # x_mode_setup and the fork's receiver setups write local artifacts only and
  # never leave the machine.
  local_phase && x_mode_setup
  local_phase && clickstack_webhook_setup
  local_phase && betterstack_webhook_setup
  local_phase && captain_msg_setup
