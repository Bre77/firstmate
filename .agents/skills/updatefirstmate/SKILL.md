...
1. **Run the updater:**
   ...
   bin/fm-update.sh
   ```
   Under the fork model it first integrates `upstream` into the fork integration line (see above), then fast-forwards this firstmate repo's default branch from origin, then updates every registered local or remote secondmate home through its placement-specific guarded path.
