...
new_fork_world() {
  ...
  mkdir -p "$w/home/state" "$w/home/data" "$w/fakebin" "$w/fake"
  : > "$w/fake/windows"
  # add_sm registers live mates in this fake tmux inventory, as new_world does.
  cat > "$w/fakebin/tmux" <<'SH'
#!/usr/bin/env bash
case "${1:-}" in
  list-windows) cat "$FM_FAKE_DIR/windows" ;;
...
test_non_fork_backward_compat() {
  ...
  assert_contains "$out" "restart-secondmates: fm-sm1" "phase (b) restart signalling unchanged"
  assert_contains "$out" "nudge-secondmates: none" "phase (b) does not also nudge a restarted mate"
