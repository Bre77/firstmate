...
# fm-spawn's fork-only per-crew memory cap shell-quotes every ship/scout launch
# inside systemd-run whenever a user systemd instance answers, so launch-shape
# assertions would depend on the host. Turn the cap off for the whole suite;
# tests/fm-crew-memory-cap.test.sh clears this to exercise the wrapper.
export FM_CREW_MEMORY_CAP=off
...
# Register arbitrary paths (not just tmproots) for removal at exit, e.g. a
# fixture written inside the checkout. Shares the tmproot cleanup registry so
# the EXIT/INT/TERM traps reap it even when the append happened in a subshell.
fm_test_track_cleanup() {
  local d
  for d in "$@"; do
    [ -n "$d" ] && printf '%s\n' "$d" >> "$FM_TEST_CLEANUP_REGISTRY"
  done
}
