...
test_routine_no_action_response_reports_faithfully() {
  local contract
  contract=$(section_9)
  assert_contains "$contract" "name the event and confirm no action is needed" \
    "section 9 does not require a plain, event-scoped routine no-action response"
  assert_not_contains "$contract" "Captain, shipshape" \
    "section 9 still mandates a scripted no-action phrase regardless of what happened"
  assert_contains "$contract" "Report any unrelated decision, outcome, or finding visible in that session" \
    "section 9 does not require surfacing unrelated decisions or outcomes"
  pass "routine no-action response stays plain and does not suppress unrelated outcomes"
}
