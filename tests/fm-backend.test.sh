...
test_spawn_all_nonworktree_samples_fails_with_existing_error() {
  ...
  assert_contains "$out" "did not enter an isolated worktree" \
    "fm-spawn.sh dropped the worktree-discovery deadline refusal"

  rm -rf "/tmp/fm-$id"
  pass "fm-spawn.sh: worktree detection still refuses at its deadline when no sample ever proves out as a real worktree"
}
