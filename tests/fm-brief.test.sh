...
test_pr_body_contract_by_mode() {
  ...
  for id_proj_mode in "brief-prbody-nm-c1:no-registry-proj:no-mistakes" "brief-prbody-dp-c2:direct-proj:direct-PR"; do
    id=${id_proj_mode%%:*}
    rest=${id_proj_mode#*:}
    proj=${rest%%:*}
    mode=${rest##*:}
  ...
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" brief-prbody-lo-c4 local-proj --mode local-only >/dev/null 2>&1
...
test_code_comments_contract() {
  ...
  for id_proj_mode in "brief-comments-nm-e1:no-registry-proj:no-mistakes" "brief-comments-dp-e2:direct-proj:direct-PR" "brief-comments-lo-e3:local-proj:local-only"; do
    id=${id_proj_mode%%:*}
    rest=${id_proj_mode#*:}
    proj=${rest%%:*}
    mode=${rest##*:}
...
test_scheduled_wait_wording() {
  ...
  FM_HOME="$home" "$ROOT/bin/fm-brief.sh" brief-sched-ship firstmate --mode no-mistakes >/dev/null 2>&1 \
  ...
  assert_grep '`paused: canary deploy verification until 2026-09-12T14:30Z`' "$brief" \
