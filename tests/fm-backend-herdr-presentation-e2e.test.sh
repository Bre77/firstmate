...
# The proven-worktree spawn poll never accepts the post-create abort fixtures'
# non-worktree cwd, so it runs the full 60-try deadline; no-op only its 1s poll
# sleep so the aborts do not cost 60 real seconds, passing other durations
# (herdr lock backoff, launch settle) through so contention timing is unaffected.
...
# The abort fixtures report a non-worktree cwd. The proven-worktree spawn poll
# rejects it at the poll and hits the "did not enter a worktree within 60s"
# timeout instead of the now-unreachable validate_spawn_worktree message; the
# spawn still fails and the EXIT-trap presentation cleanup asserted below runs.
grep -F "did not enter a worktree within 60s" "$TMP_ROOT/abort-a.err" >/dev/null 2>&1 \
  || fail "post-create abort fixture A did not reach the worktree-poll timeout"
grep -F "did not enter a worktree within 60s" "$TMP_ROOT/abort-b.err" >/dev/null 2>&1 \
  || fail "post-create abort fixture B did not reach the worktree-poll timeout"
