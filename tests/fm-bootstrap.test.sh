...
test_network_phase_partitions_the_run() {
  local case_dir fakebin mask all_out skip_out only_out combined
  ...
  # node can also live in a system BASE_PATH dir on some hosts, so mask it with
  # a command()/node() override (the same technique the git-required and jq
  # cases above use) to keep the assertion host-independent.
  rm -f "$fakebin/node"
  mask="$case_dir/mask-node.bash"
  cat > "$mask" <<'SH'
command() {
  if [ "${1:-}" = -v ] && [ "${2:-}" = node ]; then
    return 1
...
SH
  ...
  all_out=$(PATH="$fakebin:$BASE_PATH" BASH_ENV="$mask" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
    FM_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/fm-bootstrap.sh")
  ...
  skip_out=$(PATH="$fakebin:$BASE_PATH" BASH_ENV="$mask" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
  ...
  only_out=$(PATH="$fakebin:$BASE_PATH" BASH_ENV="$mask" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
  ...
  [ "$(PATH="$fakebin:$BASE_PATH" BASH_ENV="$mask" FM_HOME="$case_dir/home" FM_ROOT_OVERRIDE="$case_dir/home" \
