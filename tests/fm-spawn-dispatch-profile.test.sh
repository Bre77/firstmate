...
SH
  ...
  # Stub systemd-run unavailable so the per-crew memory cap never wraps the
  # launch; these assertions are about the raw dispatch launch, a concern
  # orthogonal to the memory cap (tests/fm-crew-memory-cap.test.sh owns that).
...
test_no_profile_keeps_claude_profile_defaults() {
  ...
  expected="env -u CURSOR_AGENT -u CURSOR_INVOKED_AS CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude --dangerously-skip-permissions --settings '{\"attribution\":{\"commit\":\"\"}}' \"\$('${ROOT}/bin/fm-operational-input.sh' encode launch-brief < '$HOME_DIR/data/$id/brief.md')\""
...
test_claude_threads_model_and_effort() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "claude --dangerously-skip-permissions --settings '{\"attribution\":{\"commit\":\"\"}}' --model 'sonnet' --effort 'high'" \
