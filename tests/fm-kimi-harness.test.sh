...
test_existing_launch_templates_are_byte_pinned() {
  assert_source_line "    claude) printf '%s' 'CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude --dangerously-skip-permissions --settings '\\''{\"attribution\":{\"commit\":\"\"}}'\\'' __MODELFLAG____EFFORTFLAG__\"\$(__OPINPUT__ encode launch-brief < __BRIEF__)\"' ;;"
...
test_watcher_scopes_moon_spinner_to_recorded_kimi_task() (
  ...
  busy_state_is_busy unknown "$busy_capture" fake \
    || fail "fm-watch did not recognize the real Kimi spinner-line shape"
  printf 'window=fake\nharness=codex\n' > "$state/kimi-watch.meta"
  if busy_state_is_busy unknown "$busy_capture" fake; then
    fail "fm-watch applied Kimi's real spinner signature to a recorded Codex task"
  fi
  printf 'window=fake\nharness=kimi\n' > "$state/kimi-watch.meta"
  if busy_state_is_busy unknown 'ordinary response ending with 🌕' fake; then
    fail "fm-watch treated an ordinary Kimi moon as a spinner line"
  fi
  if busy_state_is_busy unknown '🌕 Full moon details' fake; then
    fail "fm-watch treated moon-led Kimi output without the middot separator as busy"
  fi
  if busy_state_is_busy unknown 'auto  K2.7 Coding thinking  /some/path' fake; then
    fail "fm-watch treated Kimi's idle thinking-effort status label as busy"
  fi
  if busy_state_is_busy unknown 'Ctrl+c:cancel' fake; then
