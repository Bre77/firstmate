...
Every commit rides under the captain's own identity, on every repo; fleet work is identified by metadata instead, the `fm/<task-id>` branch prefix plus the PR label (`bin/fm-spawn.sh` suppresses the Claude harness's own injected co-author trailer per crewmate launch; docs/configuration.md "PR label" owns the labelling mechanism).
...
The durable records this contract names are `data/backlog.md`, `data/projects.md`, `data/secondmates.md`, `data/captain.md`, optional `data/captain-shared.md`, `data/learnings.md`, and per-task `data/<id>/brief.md` and `data/<id>/report.md`.
Treat `data/captain.md` as the domain-local record of captain preferences, `data/captain-shared.md` as the main-authoritative shared captain-preference file for secondmate inheritance, and `data/learnings.md` as curated home-local knowledge, regardless of harness memory.

The runtime records this contract names are per-task `state/<id>.meta`, `state/<id>.status`, the steering inbox `state/<id>.inbox/`, and the slow poll `state/<id>.check.sh`, plus fleet-wide `state/.wake-queue`, the away-mode flag `state/.afk`, and the captain-note inbox `state/inbox/`.
A `state/<id>.status` line is a wake event appended by the worker, not current-state truth; `bin/fm-crew-state.sh` owns current-state reconciliation.
Every other file under `state/` is watcher, sub-supervisor, lease, supervision-branch, poll-registration, process-event, or Relay internals owned by its producing script: read one only when that script's header sends you there, and never hand-edit or hand-delete it.
`config/` holds this home's local operating choices; `docs/configuration.md` owns every one of them, including which are inherited by secondmate homes.
...
The digest makes no external-network call on its blocking path.
Every network check a session start owes runs in a bounded deferred worker owned by `bin/fm-startup-network.sh` and is reported in the digest's own `NETWORK CHECKS` section, as is the locked startup inactive-outcome scan, whose findings arrive through the ordinary durable wake queue.
...
The digest acquires the lock before anything mutates shared state, and its bootstrap sweeps, wake-queue drain, and stale-projection cleanup run only when this session actually holds that lock.
It never starts supervision: the emitted harness protocol it prints owns the exact wait or wake mechanism.
Its per-task endpoint-liveness line and status tails are presence and wake-event signals, not current state; read current state with `bin/fm-crew-state.sh <id>` when action depends on it.
Section 8 owns how to handle the drained wakes and the `OPEN DECISIONS`, `UNREAD STATUS`, and `RECORD DIVERGENCE` sections the drain prints with them.
...
Firstmate alone resolves a matched profile array, at the intake that matched it.
Load `quota-array-dispatch` before choosing among one; that skill is the single owner of the selection procedure, and `quota-axi` remains data-only.
Account for every candidate with the evidence used to keep, block, or rank it, including remaining uncertainty; never omit a candidate, guess, fall back silently, or call the result quota-informed without that accounting.
...
The effort fallback and its precedence are owned by `harness-adapters`; do not add model-specific versions of that policy.
...
An ask-user finding returns as `needs-decision`; the implementation worker never answers its own finding, so firstmate loads `ask-user-authority` and either decides or escalates per that skill.
...
Keep exactly one live supervision cycle, using the emitted protocol for this primary harness, whenever this home has work in flight, a Relay poll, a registered process-to-event source, or an armed custom check; the last three require that cycle even with no fleet work.
...
For a genuine emergency or outage, firstmate may additionally page the captain's phone via `bin/fm-notify-captain.sh` (fork-only).

Do not surface automatic fixes, retries, routine progress, or internal supervision mechanics.
When a routine operational update's specific event requires no action but a response must be sent, reply briefly and plainly - name the event and confirm no action is needed - rather than a scripted phrase that would fit regardless of what happened.
Report any unrelated decision, outcome, or finding visible in that session that the captain would want to know, even when it needs no action, per prime directive 5's report-outcomes-faithfully mandate.
...
For a fork-only change that lives only on this fork and is never upstreamed, scaffold with `--fork-only`; the scaffold overrides the registered delivery mode with the fork-only contract (branch off the fork's main, validate locally, deliver via `bin/fm-fork-deliver.sh`, firstmate folds), because this clone's no-mistakes gate always targets upstream and cannot retarget the fork - see `docs/fork-only-delivery.md`.
...
- `captain-msg-response` - load on a `check:` wake whose output names `captain-msg` to triage inbound Twilio RCS reply payloads in `state/captain-msg-inbox/` and act; fork-only, relevant only when the captain messaging channel is enabled (`docs/captain-messaging.md`).
