...
A native `busy` verdict suppresses stale detection outright, while a settled `idle` verdict with no rendered busy banner in that harness-scoped fallback dedupes stale wakes on the semantic `agentstate:idle` signal instead of volatile pane content, so unknown or still-busy-looking panes fall back to the same pane hash path as tmux.
