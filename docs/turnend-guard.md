...
An armed `state/<id>.check.sh` also requires supervision even after its owning task's metadata is gone, covering both an `fm-pr-check` merge or activity poll and a hand-registered check such as a daily sweep; `fm_sup_check_armed` in `bin/fm-supervision-lib.sh` reuses `bin/fm-watch.sh`'s own PR-poll and check-trust validity predicates, so a quarantined or trust-failed check never counts.
