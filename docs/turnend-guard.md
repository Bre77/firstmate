...
A fresh leftover beacon blocks when the lock is missing, dead, or identity-mismatched, unless the beacon is still within the re-arm grace window described below.
...
`tests/fm-watcher-lock.test.sh` covers the lock-identity write-side and read-side race fixes recorded in [`verification/supervision.md`](verification/supervision.md#turn-end-guard).
