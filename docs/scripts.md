...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`), scout, secondmate-charter, fork-only delivery, and Herdr-lab briefs, with Captain's intent and Firstmate spec subsections on ship/scout |
...
| `fm-fork-deliver.sh`     | Deliver a fork-only change: run the local quality gate, push to the fork remote, and open a PR into the fork, never upstream (docs/fork-only-delivery.md) |
...
| `fm-pr-lib.sh`           | Own canonical task and PR validation, private atomic PR-poll publication, merge-notification identity, identity-bound retirement, and the best-effort fleet-provenance PR label (docs/configuration.md "PR label") |
| `fm-pr-poll.sh`          | Provide the byte-static watcher program for validated PR/MR-poll sidecars           |
| `fm-pr-check.sh`         | Record validated `pr=` and `pr_head=` values, atomically arm a static merge poll, and apply the fleet-provenance PR label |
| `fm-pr-activity-poll.sh` | Fork-only: surface new PR comments/reviews from anyone since a durable per-task watermark, run by the watcher whenever the merge poll finds the PR still open |
