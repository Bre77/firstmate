...
A healthy-watcher snapshot can go stale between the check and the moment an arm commits to reporting it: the target can die in the gap, e.g. while `state/.watch-cycle-exits.log` lock waits or ledger writes run.
Every site that is about to claim an attach re-verifies immediately beforehand that `state/.watch.lock` still exists AND the pid still resolves, never trusting an earlier snapshot.
A target that died in that gap falls through to a fresh start at the top-level attach check, or to the existing no-successor handling everywhere else in the cycle - never a false `watcher: attached` report.

The watcher's own HUP/INT/TERM traps reflect the signal in the exit code (128+signum), the same convention the arm's traps already use, so a process-group signal that ends the watcher directly is never indistinguishable from a genuine internal failure.
The arm classifies a signal-ended child as `watcher: FAILED - watcher cycle ended by signal <name> (<code>), not an internal error; the next arm attempt recovers it` instead of the unqualified `exited $rc without an actionable reason` wording, which otherwise reads as an unexplained failure for what is usually benign teardown between turns.
...
`tests/fm-watch-arm.test.sh` covers `attach_target_verified` directly (sourcing the script's function definitions without running it) - a live pid with an existing lock passes, a dead pid with its stale lock still present fails, and a live pid with no lock at all fails, the lock and the pid resolution each independently required - plus durable queue replay, real remote parent-replies ingestion into the authoritative status log, decision-only OPEN DECISIONS recovery, interrupted handling replay, generation-bound acknowledgement, a persistent live successor after recovery, a watcher close inside the handling window that must leave the printed acknowledgement valid, and the self-healing moved-generation acknowledgement that consumes its handled rows and names its remedy.
`tests/fm-watcher-lock.test.sh` covers verified-successor attach, recovery publication before stale-lock removal, the typed self-eviction failure, bounded and successor-linked lifecycle rows, a SIGSTOP counterfactual that distinguishes a live PID from a stale beacon before classifying termination, and a watcher signaled directly (TERM) exiting with the reflected 128+signum code and the arm reporting it as a signal, not an unexplained internal failure.
